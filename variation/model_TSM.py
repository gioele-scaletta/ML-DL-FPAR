...
class selfAttentionModel(nn.Module):
    ...
    def forward(self, inputVariable):
        ...
        attentionFeat = attentionFeat.view(-1, nf, nc*h*w)
        attentionFeat = attentionFeat.mean(1)
        attentionFeat = attentionFeat.view(-1,nc,h,w)
        x = self.avgpool(attentionFeat)
        x = x.view(x.size(0), -1)
        final_logit = self.fc(x)
